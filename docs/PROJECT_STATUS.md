...
- 상대 프로필에서 `1:1 메시지` 탭을 선택하면 큰 프로필 헤더를 즉시 접어 대화 목록이 화면 높이를
  ...
  돌아가면 프로필 헤더를 복원한다.
- 키보드가 열리면 앱 창이 IME 위까지 자동으로 줄어드는 기기 동작을 그대로 사용하고, 루트 하단 탐색 바의
  예약 공간만 해제한다. 대화 화면에서 키보드 높이를 중복으로 더하지 않아 입력란과 개인정보 경고·글자 수가
  키보드 바로 위에 붙고, 대화 목록은 실제 남은 높이에 맞춰 마지막 메시지로 이동한다.
